...
const getPercentage = (count) => {
    return props.totalKrews > 0 ? Math.round((count / props.totalKrews) * 100) : 0;
};
...
<template>
    <div class="card h-full flex flex-col">
        <div class="flex justify-between items-center mb-6">
            ...
            <Button v-if="hasMore" label="전체 보기" icon="pi pi-external-link" text size="small" @click="showAllStats" />
        </div>

        <ul class="list-none p-0 m-0 flex-1">
            <li v-for="(stat, index) in topCorpStats" :key="index" class="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
                <div>
                    <span class="text-surface-900 dark:text-surface-0 font-medium mr-2 mb-1 md:mb-0">
                        {{ stat.corp }}
                    </span>
                    <div class="mt-1 text-muted-color">{{ stat.count.toLocaleString() }}명</div>
                </div>
                <div class="mt-2 md:mt-0 flex items-center">
                    <div class="bg-surface-300 dark:bg-surface-500 rounded-border overflow-hidden w-40 lg:w-24" style="height: 8px">
                        <div :class="`${getCorpColorClass(stat.corp)} h-full`" :style="`width: ${getPercentage(stat.count)}%`"></div>
                    </div>
                    <span :class="`${getCorpTextColorClass(stat.corp)} ml-4 font-medium`"> {{ getPercentage(stat.count) }}% </span>
                </div>
            </li>
        </ul>
    </div>
    ...
    <Dialog v-model:visible="allStatsDialog" :style="{ width: '700px', maxHeight: '85vh' }" header="전체 법인 통계" :modal="true">
            ...
            <div v-for="(stat, index) in corpStats" :key="index" class="flex items-center justify-between p-4 border border-surface-200 rounded-lg hover:bg-surface-50 dark:hover:bg-surface-800 transition-colors">
                <div class="flex-1">
                    <div class="flex items-center gap-3 mb-3">
                        ...
                        <Tag :value="`${getPercentage(stat.count)}%`" :severity="getPercentage(stat.count) > 30 ? 'success' : 'info'" />
                    </div>
                    <div class="text-muted-color mb-2">{{ stat.count.toLocaleString() }}명 / {{ totalKrews.toLocaleString() }}명</div>
                    <div class="bg-surface-300 dark:bg-surface-500 rounded-border overflow-hidden" style="height: 8px">
                        <div :class="`${getCorpColorClass(stat.corp)} h-full transition-all duration-300`" :style="`width: ${getPercentage(stat.count)}%`"></div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <template #footer>
            <Button label="닫기" icon="pi pi-times" text @click="allStatsDialog = false" />
        </template>
    </Dialog>
</template>
